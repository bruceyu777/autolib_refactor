"""
Auto-generated pytest test for QAID 730265

Original DSL: ../testcase/ips/topology1/730265.txt
...
def test__730265(testbed):
    ...
    with testbed.device('FGT_A') as fgt_a:
        ...
        fgt_a.execute("sleep 22").expect('dropped(.*?)dstport=80(.*?)direction=\\"incoming\\"(.*?)Eicar.Virus.Test.File', qaid="730265")
        fgt_a.execute("dia user banned-ip list").expect('PC_05:IP_ETH1 (.*?)IPS', qaid="730265")
        fgt_a.execute("sleep 130")
        fgt_a.execute("dia user banned-ip list").expect('PC_05:IP_ETH1', qaid="730265", should_fail=True)
        fgt_a.execute("""
    config firewall proxy-policy
    del 1
    end
    ...
    purge
        ...
        fgt_a.execute("end")
        helper_outvdom(fgt_a)
        fgt_a.execute("report 730265")

    # All assertions tracked under QAID 730265
    testbed.results.report("730265")
