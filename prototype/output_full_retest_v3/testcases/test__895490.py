"""
Auto-generated pytest test for QAID 895490

Original DSL: ../testcase/ips/topology1/895490.txt
...
def test__895490(testbed):
    """
    NGFW: verify that predefined IPS signature detection is good - HTTPS
    ...
    with testbed.device('PC_02') as pc_02:
        ...
        pc_02.execute("wget -t 1 -T 15 https://PC_02:DESTINATION1/virus/eicar --no-check-certificate")
        pc_02.execute("sleep 20").expect('Connection reset by peer', qaid="895490")
    with testbed.device('FGT_A') as fgt_a:
        helper_govdom1(fgt_a)
        fgt_a.execute("exe log filter device FGT_A:LOG_DEVICE")
        fgt_a.execute("exe log filter category 4")
        fgt_a.execute("exe log display")
        fgt_a.execute("sleep 2").expect('reset(.*?)service=\\"HTTPS(.*?)policyid=99(.*?)dstport=443(.*?)direction=\\"incoming\\"(.*?)ips-ngfw(.*?)Eicar.Virus.Test.File', qaid="895490")
        fgt_a.execute("exe log filter category 0")
        fgt_a.execute("exe log display")
        fgt_a.execute("sleep 2").expect('policyid=99(.*?)policytype=\\"security-policy(.*?)service=\\"HTTPS(.*?)utmaction=\\"reset(.*?)countips=1', qaid="895490")
        fgt_a.execute("""
    config firewall security-policy
    edit 99
    unset ips-sensor
    ...
    del "ips-ngfw"
        ...
        """)
        helper_outvdom(fgt_a)
        fgt_a.execute("report 895490")

    # All assertions tracked under QAID 895490
    testbed.results.report("895490")
