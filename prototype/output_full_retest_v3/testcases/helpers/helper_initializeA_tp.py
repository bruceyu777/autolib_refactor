...
def helper_initializeA_tp(fgt):
    """
    Auto-generated from: /home/fosqa/autolibv3/autolib_v3/testcase/ips/topology1/initializeA-tp.txt

    Original DSL:
        # per vdom configration for FGT-A in TP mode
        ...
        config application custom
            edit "custom-app"
                ...
                set signature "F-SBID( --name \"test_custom\"; --vuln_id 9998; --attack_id 9652; --protocol tcp; --default_action pass; --revision 3029; --app_cat 12; --technology 1; --pop High; --risk Medium; --severity info; --app Other; --os All; --status disable; --service HTTP; --flow from_server; --file_type PDF; )"
            next
        end
        ...
        #config router static
        #    	edit 1
        #        	set gateway FGT_A:GATEWAY_ROOT
        #    	next
        #end
        ...
        #####################################################
        #  This is for exiting the root vdom
        include testcase/GLOBAL:VERSION/ips/topology1/outvdom.txt
    ...
    from .helper_goroot import helper_goroot
    from .helper_govdom1 import helper_govdom1
    ...
    fgt.execute("config router static")
    fgt.execute("edit 1")
    fgt.execute("set gateway FGT_A:GATEWAY_VD1")
    fgt.execute("next")
    fgt.execute("end")
    ...
    fgt.execute("edit \"u1\"")
    fgt.execute("set type password")
    fgt.execute("set passwd PC_05:PASSWORD")
    fgt.execute("next")
    fgt.execute("end")
    fgt.execute("config user setting")
    fgt.execute("set auth-type http https ftp telnet")
    fgt.execute("end")
    fgt.execute("config user group")
    fgt.execute("edit \"vd1_local_grp\"")
    fgt.execute("set member \"u1\"")
    fgt.execute("next")
    fgt.execute("end")
    ...
    fgt.execute("set signature \"F-SBID( --name \\\"test_custom\\\"; --vuln_id 9998; --attack_id 9652; --protocol tcp; --default_action pass; --revision 3029; --app_cat 12; --technology 1; --pop High; --risk Medium; --severity info; --app Other; --os All; --status disable; --service HTTP; --flow from_server; --file_type PDF; )\"")
    fgt.execute("next")
    fgt.execute("end")
    ...
    fgt.execute("set comment ''")
    fgt.execute("set signature \"F-SBID( --name \\\"test\\\"; --attack_id 6406; --severity low; --protocol tcp; --pattern \\\"ABCDEFG\\\";)\"")
    fgt.execute("next")
